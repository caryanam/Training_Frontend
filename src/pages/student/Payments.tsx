...
export default function StudentPayments() {
  ...
  const payments = student ? store.getPaymentsForStudent(student.id) : [];
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-accent/40 transition-colors">
                      ...
                      <td className="px-5 py-4 text-right">
                        <button
                          type="button"
                          onClick={() => alert(`Receipt downloaded for transaction ${p.transaction_id}`)}
                          className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-background px-2.5 py-1 text-xs font-medium text-muted-foreground hover:text-primary hover:border-primary transition-colors"
                        >
